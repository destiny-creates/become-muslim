package org.spongycastle.crypto.parsers;

import java.io.IOException;
import java.io.InputStream;
import org.spongycastle.crypto.KeyParser;
import org.spongycastle.crypto.params.AsymmetricKeyParameter;
import org.spongycastle.crypto.params.ECDomainParameters;
import org.spongycastle.crypto.params.ECPublicKeyParameters;

public class ECIESPublicKeyParser implements KeyParser {
    /* renamed from: a */
    private ECDomainParameters f27151a;

    public ECIESPublicKeyParser(ECDomainParameters eCDomainParameters) {
        this.f27151a = eCDomainParameters;
    }

    /* renamed from: a */
    public AsymmetricKeyParameter mo5787a(InputStream inputStream) {
        byte[] bArr;
        int read = inputStream.read();
        switch (read) {
            case 0:
                throw new IOException("Sender's public key invalid.");
            case 2:
            case 3:
                bArr = new byte[(((this.f27151a.m35647a().mo6540a() + 7) / 8) + 1)];
                break;
            case 4:
            case 6:
            case 7:
                bArr = new byte[((((this.f27151a.m35647a().mo6540a() + 7) / 8) * 2) + 1)];
                break;
            default:
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("Sender's public key has invalid point encoding 0x");
                stringBuilder.append(Integer.toString(read, 16));
                throw new IOException(stringBuilder.toString());
        }
        bArr[0] = (byte) read;
        inputStream.read(bArr, 1, bArr.length - 1);
        return new ECPublicKeyParameters(this.f27151a.m35647a().m28367a(bArr), this.f27151a);
    }
}
