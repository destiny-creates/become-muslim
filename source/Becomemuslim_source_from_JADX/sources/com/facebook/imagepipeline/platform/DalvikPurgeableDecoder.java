...
abstract class DalvikPurgeableDecoder implements PlatformDecoder {
    ...
    public CloseableReference<Bitmap> decodeFromEncodedImage(EncodedImage encodedImage, Config config, Rect rect) {
        ...
        try {
            config = pinBitmap(decodeByteArrayAsPurgeable(byteBufferRef, config));
            return config;
        } finally {
    ...
    public CloseableReference<Bitmap> decodeJPEGFromEncodedImage(EncodedImage encodedImage, Config config, Rect rect, int i) {
        ...
        Preconditions.checkNotNull(byteBufferRef);
        try {
            config = pinBitmap(decodeJPEGByteArrayAsPurgeable(byteBufferRef, i, config));
            return config;
        } finally {
            CloseableReference.closeSafely(byteBufferRef);
        }
    ...
    protected static boolean endsWithEOI(CloseableReference<PooledByteBuffer> closeableReference, int i) {
        ...
        if (i >= 2 && pooledByteBuffer.read(i - 2) == (byte) -1 && pooledByteBuffer.read(i - 1) == -39) {
            return true;
        }
        return false;
    }
