...
public class ProducerSequenceFactory {
    ...
    public Producer<CloseableReference<PooledByteBuffer>> getEncodedImageProducerSequence(ImageRequest imageRequest) {
        ...
        imageRequest = imageRequest.getSourceUriType();
    ...
    public Producer<Void> getEncodedImagePrefetchProducerSequence(ImageRequest imageRequest) {
        ...
        switch (sourceUriType) {
            ...
            default:
                ...
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("Unsupported uri scheme for encoded image fetch! Uri is: ");
                stringBuilder.append(getShortenedUriString(imageRequest));
                throw new IllegalArgumentException(stringBuilder.toString());
        }
    }
    ...
    private static void validateEncodedImageRequest(ImageRequest imageRequest) {
        ...
        Preconditions.checkArgument(imageRequest.getLowestPermittedRequestLevel().getValue() <= RequestLevel.ENCODED_MEMORY_CACHE.getValue() ? true : null);
    }
    ...
    public Producer<CloseableReference<CloseableImage>> getDecodedImageProducerSequence(ImageRequest imageRequest) {
        ...
        return this.mUseBitmapPrepareToDraw != null ? getBitmapPrepareSequence(basicDecodedImageSequence) : basicDecodedImageSequence;
    }

    public Producer<Void> getDecodedImagePrefetchProducerSequence(ImageRequest imageRequest) {
        imageRequest = getBasicDecodedImageSequence(imageRequest);
        if (this.mUseBitmapPrepareToDraw) {
            imageRequest = getBitmapPrepareSequence(imageRequest);
        }
        return getDecodedImagePrefetchSequence(imageRequest);
    }
    ...
    private Producer<CloseableReference<CloseableImage>> getBasicDecodedImageSequence(ImageRequest imageRequest) {
        ...
        switch (imageRequest) {
            ...
            case 4:
                if (MediaUtils.isVideo(this.mContentResolver.getType(sourceUri)) != null) {
                    return getLocalVideoFileFetchSequence();
                }
    ...
    private Producer<EncodedImage> newDiskCacheSequence(Producer<EncodedImage> producer) {
        if (this.mPartialImageCachingEnabled) {
            producer = this.mProducerFactory.newDiskCacheWriteProducer(this.mProducerFactory.newPartialDiskCacheProducer(producer));
        } else {
            producer = this.mProducerFactory.newDiskCacheWriteProducer(producer);
        }
        return this.mProducerFactory.newDiskCacheReadProducer(producer);
    }
    ...
    private Producer<EncodedImage> newLocalTransformationsSequence(Producer<EncodedImage> producer, ThumbnailProducer<EncodedImage>[] thumbnailProducerArr) {
        producer = this.mProducerFactory.newThrottlingProducer(this.mProducerFactory.newResizeAndRotateProducer(ProducerFactory.newAddImageTransformMetaDataProducer(producer), true, this.mUseDownsamplingRatio));
        ProducerFactory producerFactory = this.mProducerFactory;
        return ProducerFactory.newBranchOnSeparateImagesProducer(newLocalThumbnailProducer(thumbnailProducerArr), producer);
    }
    ...
    private static String getShortenedUriString(Uri uri) {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(uri.substring(0, 30));
