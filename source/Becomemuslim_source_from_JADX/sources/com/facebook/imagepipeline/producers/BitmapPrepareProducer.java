                    ...
                    if (closeableImage != null) {
                        if (!closeableImage.isClosed()) {
                            if (closeableImage instanceof CloseableStaticBitmap) {
                                closeableReference = ((CloseableStaticBitmap) closeableImage).getUnderlyingBitmap();
                                if (closeableReference != null) {
                                    int rowBytes = closeableReference.getRowBytes() * closeableReference.getHeight();
                                    if (rowBytes >= this.mMinBitmapSizeBytes && rowBytes <= this.mMaxBitmapSizeBytes) {
                                        closeableReference.prepareToDraw();
                                    }
                                }
                            }
                        }
