...
public class DeviceAuthDialog extends C4457h {
    ...
    class C18213 implements Runnable {
        C18213() {
        }
    ...
    class C18225 implements DialogInterface.OnClickListener {
        C18225() {
        }
    ...
    private static class RequestState implements Parcelable {
        public static final Creator<RequestState> CREATOR = new C18241();
        ...
        static class C18241 implements Creator<RequestState> {
            C18241() {
            }
    ...
    class C37721 implements Callback {
        C37721() {
        }
    ...
    class C37734 implements Callback {
        C37734() {
        }
        ...
        public void onCompleted(GraphResponse graphResponse) {
            if (!DeviceAuthDialog.this.completed.get()) {
                ...
                try {
                    graphResponse = graphResponse.getJSONObject();
                    DeviceAuthDialog.this.onSuccess(graphResponse.getString("access_token"), Long.valueOf(graphResponse.getLong(AccessToken.EXPIRES_IN_KEY)), Long.valueOf(graphResponse.optLong(AccessToken.DATA_ACCESS_EXPIRATION_TIME)));
                } catch (Throwable e) {
                    DeviceAuthDialog.this.onError(new FacebookException(e));
                }
            }
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        this.dialog = new Dialog(getActivity(), C1729R.style.com_facebook_auth_dialog);
    ...
    public void startLogin(Request request) {
        ...
        request = request.getDeviceAuthTargetUserId();
        if (request != null) {
            bundle.putString(DeviceRequestsHelper.DEVICE_TARGET_USER_ID, request);
        }
        ...
        new GraphRequest(null, DEVICE_LOGIN_ENDPOINT, bundle, HttpMethod.POST, new C37721()).executeAsync();
    }
    ...
    protected View initializeContentView(boolean z) {
        z = getActivity().getLayoutInflater().inflate(getLayoutResId(z), null);
        this.progressBar = z.findViewById(C1729R.id.progress_bar);
        this.confirmationCode = (TextView) z.findViewById(C1729R.id.confirmation_code);
        ((Button) z.findViewById(C1729R.id.cancel_button)).setOnClickListener(new C18202());
        this.instructions = (TextView) z.findViewById(C1729R.id.com_facebook_device_auth_instructions);
        this.instructions.setText(Html.fromHtml(getString(C1729R.string.com_facebook_device_auth_instructions)));
        return z;
    }

    protected int getLayoutResId(boolean z) {
        return z ? C1729R.layout.com_facebook_smart_device_dialog_fragment : C1729R.layout.com_facebook_device_auth_dialog_fragment;
    }
    ...
    private void schedulePoll() {
        this.scheduledPoll = DeviceAuthMethodHandler.getBackgroundExecutor().schedule(new C18213(), this.currentRequestState.getInterval(), TimeUnit.SECONDS);
    }

    private GraphRequest getPollRequest() {
        Bundle bundle = new Bundle();
        bundle.putString("code", this.currentRequestState.getRequestCode());
        return new GraphRequest(null, DEVICE_LOGIN_STATUS_ENDPOINT, bundle, HttpMethod.POST, new C37734());
    }

    private void presentConfirmation(String str, PermissionsPair permissionsPair, String str2, String str3, Date date, Date date2) {
        CharSequence string = getResources().getString(C1729R.string.com_facebook_smart_login_confirmation_title);
        String string2 = getResources().getString(C1729R.string.com_facebook_smart_login_confirmation_continue_as);
        CharSequence string3 = getResources().getString(C1729R.string.com_facebook_smart_login_confirmation_cancel);
        ...
        }).setPositiveButton(string3, new C18225());
        builder.create().show();
    }
            ...
            public void onCompleted(GraphResponse graphResponse) {
                if (!DeviceAuthDialog.this.completed.get()) {
                    ...
                    try {
                        ...
                        String string2 = graphResponse.getString("name");
                        DeviceRequestsHelper.cleanUpAdvertisementService(DeviceAuthDialog.this.currentRequestState.getUserCode());
                        if (FetchedAppSettingsManager.getAppSettingsWithoutQuery(FacebookSdk.getApplicationId()).getSmartLoginOptions().contains(SmartLoginOption.RequireConfirm) == null || DeviceAuthDialog.this.isRetry != null) {
