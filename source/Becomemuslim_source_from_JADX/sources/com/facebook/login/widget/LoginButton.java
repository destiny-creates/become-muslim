...
public class LoginButton extends FacebookButtonBase {
    ...
    protected class LoginClickListener implements OnClickListener {
        ...
        protected void performLogout(Context context) {
            ...
            if (LoginButton.this.confirmLogout) {
                CharSequence string;
                CharSequence string2 = LoginButton.this.getResources().getString(C1833R.string.com_facebook_loginview_log_out_action);
                CharSequence string3 = LoginButton.this.getResources().getString(C1833R.string.com_facebook_loginview_cancel_action);
                Profile currentProfile = Profile.getCurrentProfile();
                if (currentProfile == null || currentProfile.getName() == null) {
                    string = LoginButton.this.getResources().getString(C1833R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    string = String.format(LoginButton.this.getResources().getString(C1833R.string.com_facebook_loginview_logged_in_as), new Object[]{currentProfile.getName()});
                }
    ...
    class C37832 extends AccessTokenTracker {
        C37832() {
        }
    ...
    private void checkToolTipSettings() {
        switch (this.toolTipMode) {
            case AUTOMATIC:
                ...
                return;
            case DISPLAY_ALWAYS:
                displayToolTip(getResources().getString(C1833R.string.com_facebook_tooltip_default));
                return;
            default:
                return;
        }
    ...
    protected void configureButton(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        if (isInEditMode() != null) {
            setBackgroundColor(getResources().getColor(C1729R.color.com_facebook_blue));
            this.loginText = "Continue with Facebook";
        } else {
            this.accessTokenTracker = new C37832();
        }
        setButtonText();
        setCompoundDrawablesWithIntrinsicBounds(C0631b.m2112b(getContext(), C1833R.drawable.com_facebook_button_login_logo), null, null, null);
    }
    ...
    protected int getDefaultStyleResource() {
        return C1833R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attributeSet, int i, int i2) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        context = context.getTheme().obtainStyledAttributes(attributeSet, C1833R.styleable.com_facebook_login_view, i, i2);
        try {
            this.confirmLogout = context.getBoolean(C1833R.styleable.com_facebook_login_view_com_facebook_confirm_logout, 1);
            this.loginText = context.getString(C1833R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = context.getString(C1833R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(context.getInt(C1833R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            context.recycle();
        }
    }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (str == null) {
            str = i2.getString(C1833R.string.com_facebook_loginview_log_in_button_continue);
            int measureButtonWidth = measureButtonWidth(str);
            if (resolveSize(measureButtonWidth, i) < measureButtonWidth) {
                str = i2.getString(C1833R.string.com_facebook_loginview_log_in_button);
            }
        }
        int measureButtonWidth2 = measureButtonWidth(str);
        String str2 = this.logoutText;
        if (str2 == null) {
            str2 = i2.getString(C1833R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.isCurrentAccessTokenActive()) {
            ...
            if (this.logoutText != null) {
                ...
                charSequence = resources.getString(C1833R.string.com_facebook_loginview_log_out_button);
            }
            setText(charSequence);
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            CharSequence string = resources.getString(C1833R.string.com_facebook_loginview_log_in_button_continue);
            int width = getWidth();
            if (width != 0 && measureButtonWidth(string) > width) {
                string = resources.getString(C1833R.string.com_facebook_loginview_log_in_button);
            }
            setText(string);
        }
    }
