...
class PositionAndSizeAnimation extends Animation implements LayoutHandlingAnimation {
    ...
    protected void applyTransformation(float f, Transformation transformation) {
        transformation = this.mStartX + (this.mDeltaX * f);
        float f2 = this.mStartY + (this.mDeltaY * f);
        this.mView.layout(Math.round(transformation), Math.round(f2), Math.round(transformation + (((float) this.mStartWidth) + (((float) this.mDeltaWidth) * f))), Math.round(f2 + (((float) this.mStartHeight) + (((float) this.mDeltaHeight) * f))));
    }
