...
public class FpsDebugFrameCallback extends FrameCallback {
    ...
    public void doFrame(long j) {
        if (!this.mShouldStop) {
            ...
            if (this.mDidJSUpdateUiDuringFrameDetector.getDidJSHitFrameAndCleanup(j2, j) != null) {
                this.mNumFrameCallbacksWithBatchDispatches++;
            }
            this.mNumFrameCallbacks++;
            j = getExpectedNumFrames();
            if ((j - this.mExpectedNumFramesPrev) - 1 >= 4) {
                this.m4PlusFrameStutters++;
            }
            if (this.mIsRecordingFpsInfoAtEachFrame) {
                Assertions.assertNotNull(this.mTimeToFps);
                this.mTimeToFps.put(Long.valueOf(System.currentTimeMillis()), new FpsInfo(getNumFrames(), getNumJSFrames(), j, this.m4PlusFrameStutters, getFPS(), getJSFPS(), getTotalTimeMS()));
            }
            this.mExpectedNumFramesPrev = j;
    ...
    public FpsInfo getFpsInfo(long j) {
        ...
        j = this.mTimeToFps.floorEntry(Long.valueOf(j));
        if (j == null) {
            return 0;
        }
        return (FpsInfo) j.getValue();
    }
